...
extension StreamExtSerializer<T> on Stream<T> {
  Stream<Object> serialize(
    ...
    return map(
        (item) => serializers.serialize(item, specifiedType: specifiedType));
  }

  Stream<Object> serializeWith(
      Serializers serializers, Serializer<T> serializer) {
    return map((item) => serializers.serializeWith(serializer, item));
  }

  Stream<T> deserialize(Serializers serializers,
      {FullType specifiedType = FullType.unspecified}) {
    return map(
        (item) => serializers.deserialize(item, specifiedType: specifiedType));
  }
...
extension StreamExtSerializersList<T> on Stream<Iterable<T>> {
  Stream<List<T>> toDartList() => map((items) => items.toList());
  ...
  Stream<Iterable<T>> deserializeIterable(
    ...
    return map((items) {
      return items.map((item) =>
          serializers.deserialize(item, specifiedType: specifiedType) as T);
    });
  }

  Stream<Iterable<T>> deserializeIterableWith(
      Serializers serializers, Serializer<T> serializer) {
    return map((items) {
